...
int main(){
    ...
    for (int i = 0;i < L1;i++){
        ...
        for (int j = 0;j < dati[i];j++){
            if (mat[coord_const][c] >= 1 and mat[coord_const][c] <= 3){
                while (mat[coord_const][c] >= 1 and mat[coord_const][c] <= 3){
                    c = (rand()%(end+1-start)+start);
                }
            }
            mat[coord_const][c] = dati[i];
        }
    }
    /*
...
int main(){
    ...
    file_hit.close();
    return 0;
>>>>>>> 586f9a9a598a07280daf10df2bb4d7aac9d99a3b
}