...
void dati_alunno(char**,short,short[],short[]);
...
{   // Inizio dichiarazioni varaibili:
    ...
    short scelta;
    short somma = 0;
    const short ore_obbligatorie = 1440;
    ...
    printf("Inserisci la tua classe accompagnata dalla sezione es:[2C:inf]:   ");
    scanf("%s",nameclass);
    ...
    dati_alunno(alunni,numeroalunni,condotte,giorni_assenze);
    ...
    */
    ...
    short new_classrom[quantita_alunni][numeromaterie];
    short classe_voti_completa[iterazione_totale][numeromaterie];
    ...
    if (quantita_alunni > 0)
    {
        for (int i = 0;i < quantita_alunni;i++)
        {
            ...
            printf("Inserisci il giorno di nascita dell'alunno %s: ",alunni_new[i]);
            scanf("%hi",&giorni_new[i]);
            printf("Inserisci il mese di nascita dell'alunno %s: ",alunni_new[i]);
            scanf("%hi",&mesi_new[i]);
            printf("Inserisci l'anno di nascita dell'alunno %s:  ",alunni_new[i]);
            scanf("%hi",&anni_new[i]);
            while (mesi_new[i] > 12)
            {
        ...
        dati_alunno(alunni_new,quantita_alunni,condotte_new,giorni_assenze_new);
        // qui invece popoliamo la nuova matrice della parte della classe nuova dangolgli die voti agli alunni nuovi
            ...
            for (int j = 0;j < numeromaterie;j++)
                {
                    ...
                    medie_new[i] = (float) somma / numeromaterie;
        ...
        exchange_date(giorni_assenze,giorni_assenze_new,numeroalunni,quantita_alunni);
        ...
        for (int i = 0; i < iterazione_totale - 1; i++)
        {
            for (int j = 0; j < iterazione_totale - i - 1; j++)
            {
                ...
                { 
            	    ...
            	    temp9 = giorni_assenze[j];
            	    giorni_assenze[j] = giorni_assenze[j+1];
            	    giorni_assenze[j+1] = temp9;
    ...
    */
    ...
    printf("\tREGISTRO DI CLASSE anno %i/%i\n",anno_corrente,anno_corrente_1);
    for (int i = 0;i < iterazione_totale;i++)
    {
    ...
    {
        /*
            adesso dopo aver unito tutti i vettori ed aver unito anche la matrice tramite il registro classe si potrà accedere al resoconto che si vuole
        */
        printf("A quel resoconto annuale si vuole accedere:  \n");
        for (int i = 0;i < iterazione_totale;i++)
        {
            printf("%i - %s\n",i,registro_completo[i]);
        }
        scanf("%hi",&scelta);
        printf("Nome alunno: %s\nClasse: %s\nData di nascita alunno: %hi/%hi/%hi\nEtà alunno: %hi\nAnno scolastico corrente: %hi-%hi\nGiorni assenze: %hi\nCorrispettivo ore assenze:  %hi/%hi\nMedia: %.2f\nCondotta alunno: %hi\n",registro_completo[scelta],nameclass,giorni[scelta],mesi[scelta],anni[scelta],anno_corrente - anni[scelta],anno_corrente,anno_corrente_1,giorni_assenze[scelta],giorni_assenze[scelta]*24,ore_obbligatorie,medie[scelta],condotte[scelta]);
        if (medie[scelta] < 6 && numero_insufficenze[scelta] > 3 || condotte[scelta] < 6 || (giorni_assenze[scelta]*24) > ore_obbligatorie)
        {
            printf("L'alunno %s è risultato non ammesso alla classe successiva avendo %hi debiti, con condotta %hi, superando il limite di ore si assenze per un totale di %hi/%hi\n",registro_completo[scelta],numero_insufficenze[scelta],condotte[scelta],giorni_assenze[scelta]*24,ore_obbligatorie);
        }
        else
        {
            printf("L'alunno %s è risultato ammesso alla classe successiva avendo %hi debiti, con condotta %hi, con un totale di ore di assenze di %hi/%hi\n",registro_completo[scelta],numero_insufficenze[scelta],condotte[scelta],giorni_assenze[scelta]*24,ore_obbligatorie);
        }
        for (int i = 0;i < numeromaterie;i++)
        {
...
*/
void dati_alunno(char **alunni,short numeroalunni,short condotte[],short ore_assenze[])
{
    for (int i = 0;i < numeroalunni;i++)
        {
            printf("Inserisci la condotta dell'alunno %s:  ",alunni[i]);
            scanf("%hi",&condotte[i]);
            printf("Inserisci il numero di giorni di assenza dell'alunno %s:    ",alunni[i]);
            scanf("%hi",&ore_assenze[i]);
        }
}
